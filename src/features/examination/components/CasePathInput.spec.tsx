...
const MOCK_CARE_PATHS = [
  { id: "1", code: "GENERAL_GYN", name: "General GYN", order: 1, description: null, specialty_id: "s1", organization_id: null, is_system: true, parent_id: null, episodes: [] },
  { id: "2", code: "PREGNANCY", name: "Pregnancy", order: 2, description: null, specialty_id: "s1", organization_id: null, is_system: true, parent_id: null, episodes: [] },
  { id: "3", code: "SURGERY", name: "Surgery", order: 3, description: null, specialty_id: "s1", organization_id: null, is_system: true, parent_id: null, episodes: [] },
  { id: "4", code: "INFERTILITY", name: "Infertility", order: 4, description: null, specialty_id: "s1", organization_id: null, is_system: true, parent_id: null, episodes: [] },
];
...
describe("CasePathInput", () => {
  it("renders a tab for each care path from the API", async () => {
    render(<CasePathInput {...makeProps("GENERAL_GYN")} />, { wrapper });
    await waitFor(() => {
  ...
  it("calls onChange with code when clicking an implemented path", async () => {
    ...
    render(<CasePathInput {...makeProps("PREGNANCY", onChange)} />, { wrapper });
  ...
  it("does NOT call onChange when clicking the already-active tab", async () => {
    const onChange = vi.fn();
    render(<CasePathInput {...makeProps("GENERAL_GYN", onChange)} />, { wrapper });
    await waitFor(() => screen.getByRole("button", { name: "General GYN" }));
    fireEvent.click(screen.getByRole("button", { name: "General GYN" }));
  ...
  it("opens coming-soon dialog when clicking Surgery", async () => {
    render(<CasePathInput {...makeProps("GENERAL_GYN")} />, { wrapper });
    await waitFor(() => screen.getByRole("button", { name: "Surgery" }));
