...
function DialogBody({ visit, onOpenChange, onCompleted }: Props) {
  ...
  return (
    <Dialog.Portal>
      ...
      <Dialog.Content className="fixed left-1/2 top-1/2 z-50 w-[calc(100vw-2rem)] max-w-md -translate-x-1/2 -translate-y-1/2 rounded-2xl bg-white p-6 shadow-2xl outline-none">
        ...
        <div className="mt-4 space-y-4">
          <label className="block">
            <span className="text-[11px] font-medium text-brand-black">
              {t("label")}
            </span>
            <textarea
          ...
          <div>
            <p className="text-[11px] text-gray-400">{t("categoriesLabel")}</p>
        ...
        <div className="mt-5 flex items-center justify-end gap-2">
          <Dialog.Close className="inline-flex h-9 items-center rounded-full border border-gray-200 px-3 text-xs font-medium text-gray-600 hover:bg-gray-50">
            {t("cancel")}
          </Dialog.Close>
          <button
            type="button"
