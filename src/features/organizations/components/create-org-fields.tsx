...
import { Building2, GitBranch, UserCog } from "lucide-react";
...
type SectionProps = { form: UseFormReturn<Step3Data> };

/** Organization name + specialties. */
export function OrgInfoSection({ form }: SectionProps) {
...
/** Executive title, job role, and the Doctor specialty/title block. */
export function RoleSection({ form }: SectionProps) {
  const t = useTranslations("createOrganization");
    ...
    <FieldGroup icon={<UserCog className="size-4" />} title={t("groupRole")}>
      ...
      {isDoctor && (
        <div className="flex flex-col gap-3 ps-1">
          <label className="flex flex-col gap-1.5 text-sm text-brand-black">
            <span className="font-medium">{tAuth("doctorSpecialtyLabel")}</span>
            <Controller
              control={control}
              ...
              render={({ field }) => (
                <Select value={field.value} onValueChange={field.onChange}>
                  ...
                  >
                    <SelectValue placeholder={tAuth("doctorSpecialtyPlaceholder")} />
                  </SelectTrigger>
                  <SelectContent>
...
export function BranchSection({ form }: SectionProps) {
  const t = useTranslations("createOrganization");
  const {
    register,
    formState: { errors },
