...
export type SubspecialtyOption = { code: string; name: string };

interface SubspecialtiesSelectProps {
  /** Options for the currently-selected specialty (its nested subspecialties). */
  options: SubspecialtyOption[];
  value: string[];
  onChange: (values: string[]) => void;
  placeholder?: string;
  /** Disabled when no specialty is chosen yet (no options to pick from). */
  disabled?: boolean;
  hasError?: boolean;
}

/**
 * Dependent multi-select for a doctor's subspecialties. Options are supplied by
 * the parent (the chosen specialty's nested subspecialties) — this component
 * does not fetch. Mirrors the chip/checkbox styling of SpecialtiesSelect.
 */
export function SubspecialtiesSelect({
  options,
  value,
  onChange,
