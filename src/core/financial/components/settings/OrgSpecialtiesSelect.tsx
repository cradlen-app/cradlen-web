...
interface OrgSpecialtiesSelectProps {
  /** Organization id whose specialties to list. */
  organizationId: string | null | undefined;
  /** Selected specialty UUIDs. */
  value: string[];
...
/**
 * Multi-select for a service's specialties. Lists the organization's specialties
 * by name and stores their UUIDs (`OrgSpecialty.id`) — the shape the backend's
 * `specialty_ids` expects. Distinct from the shared, code-based SpecialtiesSelect.
 */
export function OrgSpecialtiesSelect({
  organizationId,
  value,
  onChange,
  placeholder = "Select specialties",
  hasError,
}: OrgSpecialtiesSelectProps) {
  const { data: options = [], isLoading } = useOrgSpecialties(organizationId);
